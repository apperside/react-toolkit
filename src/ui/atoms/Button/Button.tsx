 ...
 */
export const BaseButtonComponent: React.FC<PropsOf<typeof ThemedButton> & ButtonProps> = ({
    variant = "primary",
    label,
    isLoading,
    iconPosition = "left",
    ...
    const customChildren = useMemo(() => {
        if (label) {
            return <Text {...baseTheme.textStyle} {...variantTheme.textStyle}>{label}</Text>
        }
        if (props.children && typeof props.children === "string") {
            return <Text {...baseTheme.textStyle} {...variantTheme.textStyle}>{props.children}</Text>
        }
...
export const Button = BaseButtonComponent as StyledComponent<"button", DefaultTheme, PropsOf<typeof ThemedButton> & ButtonProps, never>;
